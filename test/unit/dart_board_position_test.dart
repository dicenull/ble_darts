  ...
  group('DartBoard Position Calculation Logic Tests', () {
    ...
    test('should detect D-BULL in outer bull area', () {
      final position = calculatePositionFromOffset(
        Offset(center.dx + 20, center.dy), size);  // 少し右にずらす
    ...
    test('should detect S20 at top single area', () {
      ...
      final position = calculatePositionFromOffset(
        Offset(center.dx, center.dy - singleAreaRadius), size);
      print('S20 test - position: $position, offset: ${Offset(center.dx, center.dy - singleAreaRadius)}');
      expect(position, 'S20');
    });
    ...
    test('should detect D20 at top double area', () {
      ...
      final position = calculatePositionFromOffset(
        Offset(center.dx, center.dy - doubleAreaRadius), size);
      print('D20 test - position: $position, offset: ${Offset(center.dx, center.dy - doubleAreaRadius)}');
      expect(position, 'D20');
    });
    ...
    test('should detect T20 at top triple area', () {
      ...
      final position = calculatePositionFromOffset(
        Offset(center.dx, center.dy - tripleAreaRadius), size);
      print('T20 test - position: $position, offset: ${Offset(center.dx, center.dy - tripleAreaRadius)}');
      expect(position, 'T20');
    });
    ...
    test('should test various positions around the board', () {
      ...
      final sixPosition = calculatePositionFromOffset(
        Offset(center.dx + sixSingleRadius, center.dy), size);
      print('6 single test - position: $sixPosition');
      
      // 3番（下側）のダブル領域をテスト  
      final threeDblRadius = radius * 0.725;
      final threePosition = calculatePositionFromOffset(
        Offset(center.dx, center.dy + threeDblRadius), size);
      print('3 double test - position: $threePosition');
      
      // 11番（左側）のトリプル領域をテスト
      final elevenTplRadius = radius * 0.355;
      final elevenPosition = calculatePositionFromOffset(
        Offset(center.dx - elevenTplRadius, center.dy), size);
      print('11 triple test - position: $elevenPosition');
    });
  });
}